# -*- coding: utf-8 -*-
"""
修复并更新六祖法宝坛经 Gist 内容为纯净 UTF-8
"""
import os
import subprocess

GIST_ID = "2f0307037b8b00c6fe9af7d27b395398"
FILENAME = "六祖法宝坛经核心导读.md"

CONTENT = """# 《六祖法宝坛经》现代白话导读与核心旨趣

> 📖 全文在线详注与知识图谱：[《六祖法宝坛经》参修指要与全篇阅读](https://chanzong.space/books/liuzutanjing)  
...
- 典籍全篇在线研读：[《六祖法宝坛经》专页](https://chanzong.space/books/liuzutanjing)
...
def fix_gist():
    ...
    # 先删除乱码的旧 gist，再创建一个干净的
    try:
        subprocess.run(["gh", "gist", "delete", GIST_ID], capture_output=True)
    except Exception:
        pass
        
    cmd = ["gh", "gist", "create", "--public", "--desc", "《六祖法宝坛经》现代白话导读与核心旨趣 - chanzong.space", "-f", FILENAME, tmp_path]
    res = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8")
    new_url = res.stdout.strip().split("\n")[-1].strip()
    print("New Gist URL:", new_url)
