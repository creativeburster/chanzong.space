...
# Markers:
# ZEN_PERSONS ends with:  }\n];\n\nexport const ZEN_CONCEPTS
# ZEN_CONCEPTS ends with:  }\n];\n\nexport const ZEN_METHODS
# ZEN_METHODS ends with:  }\n];\n\nexport const ZEN_KOANS  (or ]\n;\n\n)
# ZEN_KOANS ends with:  }\n];\n\nexport interface FAQItem
# ZEN_FAQS ends with:  }\n];  (at end of file)

# 1. Add tuiyin person to ZEN_PERSONS
persons_marker = '}\n];\n\nexport const ZEN_CONCEPTS'
...
# Replace the last }\n]; with new_person + ]; 
# persons_idx points to the start of }\n];\n\nexport const ZEN_CONCEPTS
# We need to replace }\n]; with new_person + ];
content = content[:persons_idx] + new_person + '];\n\nexport const ZEN_CONCEPTS' + content[persons_idx + len(persons_marker):]
...
content = content[:concepts_idx] + new_concepts + '];\n\nexport const ZEN_METHODS' + content[concepts_idx + len(concepts_marker):]
...
# Check which pattern we matched
if '}\n];' in methods_marker:
    content = content[:methods_idx] + new_method + '];\n\nexport const ZEN_KOANS' + content[methods_idx + len(methods_marker):]
else:
    # Pattern was ]\n;\n\n - need different handling
    # Find the last } before ]\n;
    last_brace = content.rfind('  }', 0, methods_idx)
    content = content[:last_brace] + '  },\n' + new_method[4:] + '];\n\nexport const ZEN_KOANS' + content[methods_idx + len(methods_marker):]

# 4. Add 3 koans to ZEN_KOANS
koans_marker = '}\n];\n\nexport interface FAQItem'
...
content = content[:koans_idx] + new_koans + '];\n\nexport interface FAQItem' + content[koans_idx + len(koans_marker):]

# 5. Add 15 FAQs to ZEN_FAQS
# Find the last entry's }, before the final ];
faqs_marker = '}\n];'
...
print(f"tuiyin person: {verify.count(chr(34) + 'id' + chr(34) + ': ' + chr(34) + 'tuiyin' + chr(34))}")
print(f"huoju-siju concept: {verify.count(chr(34) + 'id' + chr(34) + ': ' + chr(34) + 'huoju-siju' + chr(34))}")
print(f"canhuoju method: {verify.count(chr(34) + 'id' + chr(34) + ': ' + chr(34) + 'canhuoju' + chr(34))}")
print(f"koan-246: {verify.count(chr(34) + 'id' + chr(34) + ': ' + chr(34) + 'koan-246' + chr(34))}")
print(f"koan-248: {verify.count(chr(34) + 'id' + chr(34) + ': ' + chr(34) + 'koan-248' + chr(34))}")
faq696 = "id: 'faq-696'"
faq710 = "id: 'faq-710'"
